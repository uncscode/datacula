...
def Mie_SD(
    ...
    if discretize:
        ...
        for i in range(_length):
            Q_ext[i], Q_sca[i], Q_abs[i], g[i], Q_pr[i], Q_back[i], Q_ratio[i] = ps.AutoMieQ(m, wavelength, dp[i], nMedium)
...
def discretize_ScatteringFunction(
    ...
    """
    Discretize the scattering function for a given aerosol size and refractive index.
    This is used to speed up the calculation of the scattering function.
    The function is cached so it can be used multiple times without recalculating.
...
def trunc_mono(
    ...
    fullOutput: boolean, optional
        Outputs Scattering efficiencies, z-axis values, and angles of integration with correction.
...
def kappa_fitting_caps_data(
    ...
    for i in tqdm(range(len(kappa_fit))):
        ...
        if blank_value:
            ...
            if truncation_bsca:
                bsca_truncation_dry[i] = bsca_correction_for_humidified_measurements(
                    kappa=kappa_fit[i, 0],
                    particle_counts=sizer_dn,
                    diameters=sizer_diameter,
                    water_activity_sizer=sizer_humidity/100,
                ...
                bsca_truncation_wet[i] = bsca_correction_for_humidified_measurements(
                    kappa=kappa_fit[i, 0],
                    particle_counts=sizer_dn,
                    diameters=sizer_diameter,
                    water_activity_sizer=sizer_humidity/100,
