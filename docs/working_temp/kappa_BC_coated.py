...
# %%
mass_fraction = np.logspace(-3,0,100)
...
file_path = '1hexanol.txt'
aiomfac_1hexanol = load_activities(file_path, a_scale=activity_scale)

file_path = 'oleic_acid.txt'
aiomfac_oleic = load_activities(file_path, a_scale=activity_scale)
...
# Volume of the monolayer
core = 150
film_volume = (4/3) * np.pi * ((core/2)**3 - ((core/2) - mono)**3)
diameter_film = convert.volume_to_length(film_volume, length_type='diameter')
insolube_BC_coated_2hexanone = kohler_calculation(
    dry_soluble_volume_fractions=[1],
    insoluble_to_soluble_diameters=[core, diameter_film],
    solute_densities=[1.5],
    soluble_aiomfac_dict=[aiomfac_2hexanone],
    activity_scale=activity_scale
)

oleic_pure = kohler_calculation(
    dry_soluble_volume_fractions=[1],
    insoluble_to_soluble_diameters=[0,350],
    solute_densities=[0.89],
    soluble_aiomfac_dict=[aiomfac_oleic],
    activity_scale=activity_scale
)
...
film_volume = (4/3) * np.pi * ((core/2)**3 - ((core/2) - mono)**3)
diameter_film = convert.volume_to_length(film_volume, length_type='diameter')
insolube_BC_coated_oleic = kohler_calculation(
    dry_soluble_volume_fractions=[1],
    insoluble_to_soluble_diameters=[core, diameter_film],
    solute_densities=[0.89],
    soluble_aiomfac_dict=[aiomfac_oleic],
...
ax.plot(sulfate_150nm_pure['diameter_total'][:-15],
        ...
        color=color_sulfate)
ax.plot(insolube_BC_coated_2hexanone['diameter_total'][:-15],
        insolube_BC_coated_2hexanone['supersat'][:-15],
        label='BC coated with 2-hexanone',
        color=color_bc)
ax.plot(insolube_BC_coated_oleic['diameter_total'][:-15],
        insolube_BC_coated_oleic['supersat'][:-15],
        label='BC coated with Oleic',
        color=color_dust_coat)
ax.plot(oleic_pure['diameter_total'][:-1],
        oleic_pure['supersat'][:-1],
        label='Oleic Acid',
