...
```bash
pylint $(find datacula/ -name "*.py" | xargs)
```

```bash
flake8 datacula/ --count
```
...
We use GitHub to develop `particula` completely in the open. Our repository is available here: [https://uncscode.github.io/datacula/](https://uncscode.github.io/datacula/).
